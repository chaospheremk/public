    ...
    param (
        ...
        [ValidateScript({ 
            if (-not (Test-Path -Path $_ -PathType Leaf)) {
                throw "Installer file not found: $_"
            }
            if ($_ -notmatch '\.msi$') {
                throw "Installer must be an MSI file: $_"
            }
            $true
        })]
            ...
            try {
                if ($PSCmdlet.ShouldProcess($computer, "Install PowerShell 7 and enable remoting")) {
                    ...
                    try {
                        ...
                        $remoteTempPath = "C:\Windows\Temp\$installerFileName"
                        Write-Verbose "[$computer] Copying installer to $remoteTempPath..."
                        
                        Copy-Item -Path $installerFile.FullName -Destination $remoteTempPath -ToSession $session -Force -ErrorAction Stop
                        ...
                        if ($AddToPath) {
                            $msiArgs.Add('ADD_PATH=1')
                        }
                        
                        if ($EnableRemoting) {
                            $msiArgs.Add('ENABLE_PSREMOTING=1')
                        }
                        ...
                        $installScript = {
                            param($MsiPath, $Arguments, $TimeoutSec)
                        ...
                        $installResult = Invoke-Command -Session $session -ScriptBlock $installScript -ArgumentList $remoteTempPath, $msiArgString, $TimeoutSeconds -ErrorAction Stop
                        ...
                        $verification = Invoke-Command -Session $session -ScriptBlock $verifyScript -ErrorAction Stop
                        ...
                        if ($EnableRemoting) {
                            ...
                            $remotingCheck = Invoke-Command -Session $session -ScriptBlock $remotingScript -ErrorAction Stop
